...
def get_sixtyeight_feature(video_path, view = False):
    ...
    ind = [str(i) + x for i in range(7) for x in ['x','y']]
    ind += [str(i) + x for i in range(8, 68) for x in ['x','y']]
    ind += ["left_eye_x", "left_eye_y", "left_iris_x", "left_iris_y", "right_eye_x", "right_eye_y", "right_iris_x", "right_iris_y", "eye_ar"]
